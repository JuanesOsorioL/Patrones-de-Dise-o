package com.example._7factoryfunctiontarea_crearlog;

import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;

@SpringBootApplication
public class Application {

    /**
     * ! Factory Function
     * Es un patrón de diseño que nos permite crear objetos o funciones de manera dinámica que serán
     * usados posteriormente en el código.
     * <p>
     * * Es útil cuando necesitamos crear objetos o funciones de manera dinámica,
     * * es decir, en tiempo de ejecución y no en tiempo de compilación.
     *
     */

//! Salida esperada
//! Colocar colores de log según el nivel
//* [INFO:2025-10-21:07] Aplicación iniciada correctamente.
//* [WARNING:2025-10-21:07] El uso de memoria está alto.
//* [ERROR:2025-10-21:07] Error de conexión a la base de datos.
    public static void main(String[] args) {
        SpringApplication.run(Application.class, args);

        Logger info = LoggerFactory.createLogger(LogLevel.INFO);
        Logger warn = LoggerFactory.createLogger(LogLevel.WARN);
        Logger error = LoggerFactory.createLogger(LogLevel.ERROR);

        info.show("informativo");
        warn.show("advertencia");
        error.show("error");


    }


}
